...
// Azure slot (they pass reliably when run serially). Retry to absorb that infra
...
test('Payment link', async ({ page }) => {
  ...
  await page.locator('button').filter({ hasText: 'Get Payment Link' }).first().click();

  // The link is generated by a backend request whose latency varies on the slow
  // Azure slot; poll for the info panel rather than using a fixed wait that the
  // (already fully valid) form can outlast on a slow day.
  const paymentLinkInfo = page.locator('.v2-class---payment-link-info');
  await expect(paymentLinkInfo).toBeVisible({ timeout: 30000 });
